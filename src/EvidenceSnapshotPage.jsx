...
export function EvidenceSnapshotPage({ snapshotId, customerId }) {
  ...
  return <main className="content-page evidence-page">
    ...
    <section className="evidence-certificate">
      ...
      <div className="evidence-platform-bar"><div><i>{String(row.platform || '平台').slice(0, 1)}</i><span><b>{row.platform}</b><small>{row.device || '设备未记录'}</small></span></div><span className={hasAnswer ? 'verified' : 'unseen'}>{hasAnswer ? <CheckCircle2 /> : <FileSearch />}{hasAnswer ? '已保存回答正文' : '未保存回答正文'}</span></div>
      ...
      {hasAnswer ? <section className="evidence-answer-section">
        <div className="evidence-answer-toolbar"><div><span>{row.platform} 回答</span><small>从用户的豆包历史会话保存</small></div><em>{captureTime}</em></div>
        ...
        <div className="evidence-source-strip">
          <div className="evidence-reference-note"><Link2 /><span><b>{referenceCount ? `参考 ${referenceCount} 篇资料` : '参考资料数量未保存'}</b><small>{referenceCount ? '数量来自采集时的豆包页面；单条引用 URL 未单独保存。' : '本地快照未保存单条引用明细。'}</small></span></div>
